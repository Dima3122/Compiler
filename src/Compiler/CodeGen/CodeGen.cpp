...
void CodeGen::visit(ASTAssign &node)
{
    if (node.get_lvalue()->get_var_type() != "")
    {
        ...
        if (node.get_lvalue()->get_var_type() == "string")
        {
            ...
            stream << "call void @llvm.memcpy.p0i8.p0i8.i64(i8* align 1 " << bitcast_ind << ", i8* align 1 getelementptr inbounds (" << type << ", " << type << "* "
                   << "@__const." << current_function << "." << node.get_lvalue()->get_var_name() << ", i32 0, i32 0), i64 " << std::to_string(node.get_rvalue1()->get_var_name().size() - 1) << ", i1 false)\n";
        }
        else
        {
        ...
        if (node.get_rvalue1() != nullptr && node.get_rvalue2() != nullptr)
        {
            ...
            if (node.get_rvalue1()->is_literal() && node.get_rvalue2()->is_literal())
            {
                if (type == "double")
                {
                    ...
                    {
                        std::cerr << "ERROR: invalid operands to binary expression\n";
                        throw;
                    }
                }
                else if (type == "i32")
